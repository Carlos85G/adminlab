...
@section("contentheader_title")
	<a href="{{ url(config('laraadmin.adminRoute') . '/prestamos') }}">Prestamo</a> :
@endsection
@section("contentheader_description", $prestamo->$view_col)
@section("section", "Prestamos")
@section("section_url", url(config('laraadmin.adminRoute') . '/prestamos'))
@section("sub_section", "Edit")

@section("htmlheader_title", "Prestamos Edit : ".$prestamo->$view_col)
			...
			<div class="col-md-8 col-md-offset-2">
				{!! Form::model($prestamo, ['route' => [config('laraadmin.adminRoute') . '.prestamos.update', $prestamo->id ], 'method'=>'PUT', 'id' => 'prestamo-edit-form']) !!}
					...
					<div class="form-group">
						{!! Form::submit( 'Update', ['class'=>'btn btn-success']) !!} <button class="btn btn-default pull-right"><a href="{{ url(config('laraadmin.adminRoute') . '/prestamos') }}">Cancel</a></button>
					</div>
				{!! Form::close() !!}
			</div>
		</div>
